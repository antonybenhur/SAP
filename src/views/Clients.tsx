...
import { Search, Plus, Download, Eye, Edit, Trash2, Building2, Users, Briefcase, Star, AlertTriangle } from 'lucide-react';
    ...
    const fetchClients = async () => {
      try {
        const { data, error } = await supabase
          .from('clients')
          .select('*, account_owner_profile:profiles(name)')
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg border border-neutral-200 dark:border-neutral-800">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400">Total Clients</p>
        ...
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg border border-neutral-200 dark:border-neutral-800">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400">Strategic Clients</p>
              <p className="text-2xl font-bold text-neutral-900 dark:text-white">
                {clients.filter(c => c.client_tier === 'strategic').length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg border border-neutral-200 dark:border-neutral-800">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400">Active Job Orders</p>
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-800 overflow-hidden">
                ...
                <tr key={client.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-neutral-900 dark:text-white">
                      {(client as any).account_owner_profile?.name || 'Not assigned'}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${tierColors[client.client_tier]}`}>
                      {client.client_tier === 'strategic' ? 'Tier 1' :
                       client.client_tier === 'active' ? 'Tier 2' :
                       client.client_tier === 'prospect' ? 'Prospect' : 'Past Client'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${statusColors[client.status]}`}>
                      {client.status.charAt(0).toUpperCase() + client.status.slice(1)}
                    </span>
                    {client.msa_status === 'expired' && (
                      <div className="flex items-center mt-1">
                        <AlertTriangle className="w-3 h-3 text-red-500 mr-1" />
                        <span className="text-xs text-red-500">MSA Expired</span>
                      </div>
                    )}
                  </td>
        ...
        {filteredClients.length === 0 && (
          <div className="p-8 text-center">
            ...
            <p className="text-neutral-500 dark:text-neutral-400">No clients found matching your criteria.</p>
            <button
              onClick={handleAddClient}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-neutral-900 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-neutral-600 dark:text-neutral-400 mb-6">
              Are you sure you want to delete this client? This action cannot be undone and will also remove all associated job orders.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => deleteConfirm && handleDeleteClient(deleteConfirm)}
